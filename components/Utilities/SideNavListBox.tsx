...
interface SideNavListBoxProps {
  ...
  /** Optional header row above the list. */
  heading?: ReactNode;
...
/**
 * The app's plain vertical sidebar list.
 *
 * NOTHING IN THIS SKELETON RENDERS IT YET — the household sidebar
 * (ListBoxComponent) is deliberately its own component, because each of its
 * rows carries per-row totals, a dropdown and rename/delete dialogs, which is
 * more than a plain list. This one is kept as the thing to reach for when you
 * add a second sidebar (a reports list, a settings section nav), so that
 * sidebar matches by construction instead of becoming a third copy of the
 * same class strings.
 *
